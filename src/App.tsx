        ...
        {cart.length > 0 && (
          <div style={{
            ...
            {(() => {
              ...
              const qualifies = itemCount >= 2;
              return (
                <div style={{
                  background: qualifies ? "rgba(34,197,94,0.1)" : "rgba(168,85,247,0.1)",
                  border: `1px solid ${qualifies ? "rgba(34,197,94,0.3)" : "rgba(168,85,247,0.3)"}`,
                  borderRadius: "10px", padding: "10px 14px", marginBottom: "16px",
                  display: "flex", alignItems: "center", gap: "8px"
                }}>
                  <span style={{ color: qualifies ? "#22c55e" : "#a855f7", display: "flex" }}>
                    {qualifies ? <IconCheck /> : <IconStar />}
                  </span>
                  <span style={{
                    fontSize: "12px", fontWeight: "600",
                    color: qualifies ? "#22c55e" : "#a855f7"
                  }}>
                    {qualifies
                      ? "Buy 2 Save 15% applied at checkout!"
                      : "Add 1 more item to save 15% on your order"}
                  </span>
                </div>
              );
            })()}
    ...
    <div style={{
      ...
      {Array(8).fill(0).map((_, i) => (
        <span key={i} style={{
          ...
          letterSpacing: "0.5px", padding: "0 28px",
          display: "inline-flex", alignItems: "center", gap: "8px"
        }}>
          BUY 2 SAVE 15% &nbsp;·&nbsp; FREE UK &amp; US SHIPPING &nbsp;·&nbsp; 2-5 DAY DELIVERY
        </span>
      ))}
    </div>
    <style>{`
...
const NavBar = ({ cartCount, onCartOpen }) => (
  <nav style={{
    position: "fixed", top: "34px", left: 0, right: 0, zIndex: 200,
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        boxShadow: `0 40px 100px ${product.color}20`
      }}>
        {/* Big image area */}
        <div style={{
          height: "240px", background: product.bg,
          ...
          <button onClick={onClose} style={{
            ...
            color: "#fff", display: "flex", alignItems: "center", zIndex: 2
          }}><IconX /></button>
          ...
          <div style={{
            ...
            color: product.badgeColor, letterSpacing: "0.5px"
          }}>{product.badge}</div>
          ...
          <div style={{
            ...
            textAlign: "center", fontSize: "10px", color: "rgba(255,255,255,0.25)"
          }}>Swap in your AliExpress product photo here</div>
        </div>

        {/* Content */}
        <div style={{ padding: "28px" }}>
          <h2 style={{ color: "#fff", fontWeight: "900", fontSize: "24px", letterSpacing: "-0.6px", marginBottom: "6px" }}>{product.name}</h2>
          <p style={{ color: product.color, fontSize: "13px", fontWeight: "600", marginBottom: "16px" }}>{product.tagline}</p>
          <p style={{ color: "rgba(255,255,255,0.5)", fontSize: "14px", lineHeight: "1.7", marginBottom: "24px" }}>{product.description}</p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px" }}>
            <span style={{ color: "#fff", fontWeight: "900", fontSize: "30px" }}>£{product.price.toFixed(2)}</span>
            <span style={{ color: "rgba(255,255,255,0.25)", fontSize: "16px", textDecoration: "line-through" }}>£{product.oldPrice.toFixed(2)}</span>
            <span style={{
              background: "#22c55e18", border: "1px solid #22c55e35",
              borderRadius: "6px", padding: "4px 10px",
              fontSize: "11px", color: "#22c55e", fontWeight: "700"
            }}>SAVE £{(product.oldPrice - product.price).toFixed(2)}</span>
          </div>

          {/* Add to cart */}
          <button onClick={handleAdd} style={{
            width: "100%",
            background: added ? "#22c55e" : `linear-gradient(135deg, ${product.color}, #a855f7)`,
            border: "none", borderRadius: "12px", padding: "16px",
            fontSize: "15px", fontWeight: "800", color: added ? "#fff" : "#000",
            cursor: "pointer", letterSpacing: "-0.3px",
            display: "flex", alignItems: "center", justifyContent: "center", gap: "8px",
            transition: "all 0.25s ease"
          }}>
            {added ? <><IconCheck /> Added to Cart</> : `Add to Cart — £${product.price.toFixed(2)}`}
          </button>

          {/* Trust line */}
          <div style={{ display: "flex", justifyContent: "center", gap: "18px", marginTop: "16px" }}>
            {[[<IconTruck />, "Free Shipping"], [<IconRefresh />, "30-Day Returns"], [<IconLock />, "Secure"]].map(([icon, label]) => (
    ...
    <div style={{
      ...
      <div id="products" style={{ maxWidth: "1200px", margin: "0 auto", padding: "80px 24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "20px" }}>
          {products.map(p => <ProductCard key={p.id} product={p} onAddToCart={addToCart} onOpen={setOpenProduct} />)}
        </div>
      </div>
